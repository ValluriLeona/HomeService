...
export default function BeautyProduct({ store }) {
  ...
  useEffect(() => {
    axios.get("http://localhost:8082/svc/beauty")
      .then((res) => {
        setResult(res.data);
        setLoading(false);
      })
      .catch((err) => {
        setError("Error fetching beauty services!");
        setLoading(false);
      });
  }, []);
  ...
  const getSortedResult = () => {
    if (sortOrder === "lowToHigh") {
      return [...result].sort((a, b) => a.pcost - b.pcost);
    } else if (sortOrder === "highToLow") {
      return [...result].sort((a, b) => b.pcost - a.pcost);
    }
    return result;
  };
  ...
  if (loading) return <div>Beauty & Wellness Services List Fetching...</div>;
