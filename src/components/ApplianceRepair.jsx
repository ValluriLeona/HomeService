...
export default function ApplianceRepair({ store }) {
  ...
  useEffect(() => {
    axios.get("http://localhost:8082/svc/appliance")
      .then((res) => {
        setResult(res.data);
        setLoading(false);
      })
      .catch(() => {
        setError("Error fetching appliance repair services!");
        setLoading(false);
      });
  }, []);
  ...
  if (loading) return <div>Appliance Repair Services Loading...</div>;
